...
FILE* out;
char buf[55],aux[70],numJ[5];
	int j = 1, k=0; //contadora
	out = fopen("readme.decifra.txt","wr"); //apenas o out nesse loop pq so vamos concatenar nele usando strcat.... de alguma forma
	fseek(out,0,SEEK_SET);
	itoa(j,numJ,10);
	while(fgets(buf,55,out)){
		strcat(aux,"Linha ");
		strcat(aux,numJ);
		strcat(aux," :");
		strcat(aux,buf);
